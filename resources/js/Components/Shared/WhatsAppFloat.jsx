...
export const WhatsAppFloat = ({ corporativeInfo = null }) => {
  const phoneNumber = corporativeInfo?.corporative_float_whatsapp || "+573115327297";
  const [isHovered, setIsHovered] = useState(false)
  const [showTooltip, setShowTooltip] = useState(false)

  const handleWhatsAppClick = () => {
    // Format the phone number for WhatsApp URL (remove + and any spaces/dashes)
    const formattedNumber = phoneNumber.replace(/[\+\s\-\(\)]/g, '')
    
    // Pre-written message for better user experience
    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        ...
        >
          ...
          {/* Ripple Effect */}
          <div className={`absolute inset-0 bg-green-400 opacity-30 animate-ping ${isHovered ? 'block' : 'hidden'}`} />
        </button>
        ...
        >
          ¡Chatea con nosotros!
          {/* Tooltip Arrow */}
          <div className="absolute top-full right-4 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent border-t-darki" />
        </div>
      </div>

      {/* Mobile-only: Additional notification badge */}
