  ...
  return (
    <MotionWrapper>
      <Link href={`/servicios/${slug || id}`}>
        <article className="flex flex-col items-center justify-between h-full p-8 text-center transition-all duration-300 border cursor-pointer border-softGrey hover:shadow-lg hover:border-golden hover:scale-105 lg:p-16 group">
          <div className="transition-colors duration-300 group-hover:text-golden">
            <IconComponent className="object-cover w-full h-16 text-golden lg:h-20" />
          </div>
          ...
          <p className="mt-2 text-sm text-greyki lg:text-base line-clamp-3">
            {description}
          </p>
          <div className='p-3 mx-auto mt-4 bg-softGrey group-hover:bg-golden transition-colors duration-300 lg:p-4 lg:mt-6'>
            <ArrowRight className="object-cover w-full text-golden group-hover:text-white transition-colors duration-300" />
          </div>
        </article>
      </Link>
    </MotionWrapper>
